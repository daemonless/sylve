...
# Install runtime packages. The bhyve bootrom is per-arch: edk2 UEFI on amd64,
# U-Boot on aarch64 (no edk2-bhyve build exists for arm64).
RUN case "${FREEBSD_ARCH}" in \
        aarch64) FIRMWARE=u-boot-bhyve-arm64 ;; \
        *)       FIRMWARE=bhyve-firmware ;; \
    esac && \
    pkg update && \
    pkg install -y ${PACKAGES} ${FIRMWARE} && \
    pkg clean -ay && \
